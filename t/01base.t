...
use_ok("version::overloaded"); # If we made it this far, we are ok.
...
$version = new version::overloaded 5.005_03;
is ( "$version" , "5.5.30" , '5.005_03 eq 5.5.30' );
$version = new version::overloaded 1.23;
...
$new_version = new version::overloaded "5.8.0";
diag "tests with objects" unless $ENV{PERL_CORE};
ok ( $version ne $new_version, '$version ne $new_version' );
ok ( $version lt $new_version, '$version lt $new_version' );
ok ( $new_version gt $version, '$new_version gt $version' );
$new_version = new version::overloaded "$version";
ok ( $version eq $new_version, '$version eq $new_version' );
...
# now test with existing object
$new_version = new version::overloaded $new_version;
...
$version = new version::overloaded "1.2.4";
...
$version = new version::overloaded "1.2.3";
$new_version = new version::overloaded "1.2.3_4";
...
$version = new version::overloaded "1.2.4";
$new_version = new version::overloaded "1.2_4";
...
$version = new version::overloaded "v1.2";
$new_version = new version::overloaded "1.2.0";
ok ( $version == $new_version, '$version == $new_version' );
$new_version = new version::overloaded "1.2_0";
ok ( $version == $new_version, '$version == $new_version' );
$new_version = new version::overloaded "1.2.1";
ok ( $version < $new_version, '$version < $new_version' );
$new_version = new version::overloaded "1.2_1";
ok ( $version < $new_version, '$version < $new_version' );
$new_version = new version::overloaded "1.1.999";
ok ( $version > $new_version, '$version > $new_version' );
...
$version = new version::overloaded $Test::More::VERSION;
eval "use Test::More $version";
unlike($@, qr/Test::More version $version required/,
	'Replacement eval works with exact version');

$version = new version::overloaded $Test::More::VERSION+0.01; # this should fail even with old UNIVERSAL::VERSION
eval "use Test::More $version";

...
Perl itself has used since 5.6.0 was released.  A leading "v" is optional, 
...
Dotted-decimal: v-strings (v1.2.3), strings with with more than one decimal
point ("1.2.3") and strings with a leading-v ("v1.2")
...
True only if the version object is a dotted-decimal version, e.g.
  ...
  version->parse('1.2.0')->is_qv;        # TRUE

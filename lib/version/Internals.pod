...
version.pm provides a robust version object that is designed to both 
...
to ease comparisons.  Version objects also employ L<overload> methods to
simplify code that needs to compare, print, etc the objects.

=head2 History

Until Perl 5.6.0 was released in March 2000, all $VERSION assignments were
made as floating point numbers, e.g.

  $VERSION = 1.020;

or in the case of an alpha/developer release to CPAN, with a single
underscore:

  $VERSION = "1.020_1";
  $VERSION = eval($VERSION); # looks like 1.0201 now

The C<eval> is required because most code (in the core an in core modules)
assumed that C<$VERSION> always contained a number.  Packaging code, like
L<ExtUtils::MakeMaker> and more recently L<Module::Build> would "see" the
first assignment and retain the underscore, e.g. in the package name.
Perl itself ignores underscores in bare (unquoted) numbers, which is what
the C<eval> is equivalent to, so anything that assumed a number would get
a number.
...
Any initial parameter which "looks like a number", see L<Decimal
Versions>.  This also covers versions with a single decimal point and
a single embedded underscore, see L<Decimal Alpha Versions>, even though
...
and an optional embedded underscore, see L<Dotted-Decimal Versions>.  This 
...
they were L<Decimal Versions>, for parsing and output purposes.  The
...
exactly as if they were L<Dotted-Decimal Versions>, and will display without any
